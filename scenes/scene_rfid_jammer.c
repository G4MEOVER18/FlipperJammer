...
static void rfid_jam_update_widget(JammerApp* app) {
    ...
    widget_add_string_element(app->widget, 64,  2, AlignCenter, AlignTop, FontPrimary,   "RFID JAMMER AKTIV");
    widget_add_string_element(app->widget,  2, 18, AlignLeft,   AlignTop, FontSecondary, "125 kHz Carrier aktiv");
    widget_add_string_element(app->widget,  2, 30, AlignLeft,   AlignTop, FontSecondary, "EM4100 / HID blockiert");
...
void jammer_scene_RfidJamRun_on_enter(void* context) {
    ...
    rfid_jam_start_carrier();
    jammer_notify_start(app);

    rfid_jam_update_widget(app);
    view_dispatcher_switch_to_view(app->view_dispatcher, ViewWidget);

    furi_timer_start(app->run_timer, 500);
}

bool jammer_scene_RfidJamRun_on_event(void* context, SceneManagerEvent event) {
    JammerApp* app     = context;
    bool       handled = false;
...
void jammer_scene_RfidJamRun_on_exit(void* context) {
    JammerApp* app = context;
